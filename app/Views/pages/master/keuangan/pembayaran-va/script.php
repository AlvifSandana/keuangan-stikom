<script>
    ...
    function acc_temp_tr(id_temp_tr, nim, q_debit) {
        ...
        // ajax req
        $.ajax({
            url: '<?= base_url() ?>/master-keuangan/va/acc',
            type: 'POST',
            data: data,
            dataType: 'JSON',
    ...
    function updateTempVA() {
        try {
            ...
            $.ajax({
                url: '<?= base_url() ?>/master-keuangan/va/update/' + $('#id_temp_transaksi').val(),
                type: 'POST',
                data: update_data,
                dataType: 'JSON',
                success: function(data) {
    ...
    function deleteTempVA(id_temp_transaksi) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url() ?>/master-keuangan/va/delete/' + id_temp_transaksi,
                    method: 'DELETE',
                    dataType: 'JSON',
                    success: function(data) {
                        if (data.status == 'success') {
    ...
    function resetTempVA() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url() ?>/master-keuangan/va/reset-tbl',
                    method: 'DELETE',
                    dataType: 'JSON',
                    success: function(data) {
                        if (data.status == 'success') {
